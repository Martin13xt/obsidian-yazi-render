                ...
                switch (_b.label) {
                  case 0:
                    ...
                    _loop_1 = function(backgroundImage3) {
                      var image2, url, _c, path2, x, y, width, height, pattern, _d, path2, x, y, width, height, _e, lineLength, x0, x1, y0, y1, canvas, ctx, gradient_1, pattern, _f, path2, left, top_1, width, height, position2, x, y, _g, rx, ry, radialGradient_1, midX, midY, f2, invF;
                        ...
                        switch (_h.label) {
                          ...
                          case 4:
                            if (image2) {
                              ...
                              ]), path2 = _c[0], x = _c[1], y = _c[2], width = _c[3], height = _c[4];
                              pattern = this_1.ctx.createPattern(this_1.resizeImage(image2, width, height), "repeat");
                              this_1.renderRepeat(path2, pattern, x, y);
                            }
                            return [3, 6];
                          case 5:
                            if (isLinearGradient(backgroundImage3)) {
                              _d = calculateBackgroundRendering(container, index, [null, null, null]), path2 = _d[0], x = _d[1], y = _d[2], width = _d[3], height = _d[4];
...
function normalizeRenderProfile(raw, fallback = "auto") {
...
function sanitizeVaultRelativePath(raw, normalizePathFn) {
...
function isRequestFresh(requestedAt, maxAgeMs, nowMs = Date.now()) {
  const requestedAtNumber = Number(requestedAt);
  if (!Number.isFinite(requestedAtNumber) || requestedAtNumber <= 0) {
    return false;
  }
  const ageMs = nowMs - requestedAtNumber * 1e3;
  if (ageMs < -3e4) {
    return false;
  }
  return ageMs <= maxAgeMs;
}
function resolveAutoRenderProfile(colsRaw, rowsRaw) {
...
var RENDER_TIMEOUT_MS = 9e3;
var CAPTURE_TIMEOUT_MS = 3e4;
var EXPORT_TIMEOUT_MS = 6e4;
var MAX_INLINE_IMAGE_PIXELS = 16e6;
var MAX_INLINE_IMAGE_EDGE = 4096;
var INLINE_IMAGE_CACHE_MAX_BYTES = 64 * 1024 * 1024;
var INLINE_IMAGE_CACHE_TTL_MS = 3e4;
var REQUEST_MAX_AGE_MS = 3 * 6e4;
var MAX_QUEUE_FILES = 128;
var MAX_CANVAS_BYTES = 512 * 1024 * 1024;
var MAX_CANVAS_EDGE = 32767;
var MIN_CAPTURE_SCALE = 0.35;
var CJK_TEXT_RE = /[\u3000-\u30ff\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff]/;
var CACHE_ROOT_ENV_VAR = "OBSIDIAN_YAZI_CACHE";
var DEBUG_INCLUDE_PATHS_ENV_VAR = "OBSIDIAN_YAZI_DEBUG_INCLUDE_PATHS";
var YaziExporterPlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
    this.settings = { ...DEFAULT_SETTINGS };
    this.inFlight = /* @__PURE__ */ new Map();
    this.pendingRequests = /* @__PURE__ */ new Map();
    this.inlineImageCache = /* @__PURE__ */ new Map();
    this.inlineImageCacheBytes = 0;
    ...
    this.addCommand({
      ...
      callback: async () => {
        try {
          ...
          if (!file) {
            new import_obsidian.Notice("Yazi Exporter: active note not found.");
            return;
          }
          await this.exportFileToCache(file.path);
        } catch (error) {
          console.error("Yazi Exporter failed", error);
          new import_obsidian.Notice("Yazi Exporter: export failed. Open console for details.");
        }
      }
    });
    this.addCommand({
  ...
  async loadSettings() {
    ...
    loaded.maxHeightPx = !Number.isFinite(maxHeight) || maxHeight <= 0 ? 0 : Math.max(1e3, Math.floor(maxHeight));
  ...
  normalizeRenderProfile(raw, fallback = "auto") {
    return normalizeRenderProfile(raw, fallback);
  }
  sanitizeVaultRelativePath(raw) {
    return sanitizeVaultRelativePath(raw, import_obsidian.normalizePath);
  }
  isRequestFresh(request) {
    return isRequestFresh(request?.requestedAt, REQUEST_MAX_AGE_MS);
  }
  resolveAutoRenderProfile(colsRaw, rowsRaw) {
    return resolveAutoRenderProfile(colsRaw, rowsRaw);
  }
  shouldIncludeSensitiveDebugPaths() {
    const raw = process.env[DEBUG_INCLUDE_PATHS_ENV_VAR]?.trim().toLowerCase();
    return raw === "1" || raw === "true" || raw === "yes" || raw === "on";
  }
  async exportRequestedToCache() {
    const cacheRoot = this.resolveEffectiveCacheRoot();
    await this.ensureCacheDirs(cacheRoot);
    const requestPath = import_node_path.default.join(cacheRoot, "requests", "current.txt");
    const requestJsonPath = import_node_path.default.join(cacheRoot, "requests", "current.json");
    const requestQueueDir = import_node_path.default.join(cacheRoot, "requests", "queue");
    ...
    const requestSource = requestPayload === currentRequest ? "current" : requestPayload === queuedRequest ? "queue" : "none";
    const requestTextStat = await import_node_fs.promises.stat(requestPath).catch(() => null);
    const requestTextFresh = Boolean(requestTextStat && Date.now() - requestTextStat.mtimeMs <= REQUEST_MAX_AGE_MS);
    const requestedPathFromText = requestTextFresh ? (await import_node_fs.promises.readFile(requestPath, "utf8").catch(() => "")).split(/\r?\n/, 1)[0]?.trim() ?? "" : "";
    const requestedPath = this.sanitizeVaultRelativePath(requestPayload?.path ?? requestedPathFromText);
    const tracePath = import_node_path.default.join(cacheRoot, "log", "request-trace.json");
    if (this.settings.enableDebugLogs) {
      ...
      await import_node_fs.promises.writeFile(tracePath, JSON.stringify(tracePayload, null, 2)).catch(() => void 0);
    }
    if (!requestedPath) {
      throw new Error("Request file is empty or missing");
    }
    return this.exportFileToCache(requestedPath, requestPayload ?? void 0);
  }
  async readRenderRequest(requestJsonPath) {
    const raw = await import_node_fs.promises.readFile(requestJsonPath, "utf8").catch(() => "");
    if (!raw.trim()) {
      return null;
    }
    try {
      ...
      const quickModeRaw = parsed.quickMode;
      if (typeof quickModeRaw === "boolean") {
        request.quickMode = quickModeRaw;
      } else if (typeof quickModeRaw === "string") {
        const raw2 = quickModeRaw.trim().toLowerCase();
        if (raw2 === "1" || raw2 === "true" || raw2 === "yes") {
          request.quickMode = true;
        } else if (raw2 === "0" || raw2 === "false" || raw2 === "no") {
      ...
      const terminalProgram = typeof parsed.terminalProgram === "string" ? parsed.terminalProgram.trim().toLowerCase() : "";
      ...
      if (renderCalcAfterColsPx !== void 0) {
      ...
      if (renderCalcAfterReadabilityPx !== void 0) {
      ...
      if (renderCalcAfterTerminalPx !== void 0) {
  ...
  async readQueuedRequest(queueDir) {
    const entries = await import_node_fs.promises.readdir(queueDir).catch(() => []);
    ...
    const stats = await Promise.all(
      jsonEntries.map(async (name) => {
        const fullPath = import_node_path.default.join(queueDir, name);
        const stat = await import_node_fs.promises.stat(fullPath).catch(() => null);
        return {
          fullPath,
    ...
    await Promise.all(overflow.map((item) => import_node_fs.promises.unlink(item.fullPath).catch(() => void 0)));
    for (const item of keep) {
      if (item.mtimeMs <= 0) {
        continue;
      }
      const request = await this.readRenderRequest(item.fullPath);
      await import_node_fs.promises.unlink(item.fullPath).catch(() => void 0);
  ...
  selectPreferredRequest(currentRequest, queuedRequest) {
  ...
  resolveVaultFile(vaultRelativePath) {
    const normalized = (0, import_obsidian.normalizePath)(vaultRelativePath);
    ...
    if (normalized !== vaultRelativePath) {
      ...
      if (normalizedFile instanceof import_obsidian.TFile) {
  ...
  rewriteImageEmbeds(markdown, sourcePath, imageNameIndex) {
    const sourceDir = sourcePath.split("/").slice(0, -1).join("/");
    return markdown.replace(/!\[\[([^\]|]+)(?:\|([^\]]+))?\]\]/g, (_all, rawTarget, rawSize) => {
  ...
  resolveEmbeddedImageFile(target, sourceDir, imageNameIndex) {
    ...
    if (fromRoot instanceof import_obsidian.TFile && this.isImageFile(fromRoot)) {
      return fromRoot;
    }
    if (sourceDir) {
      const joined = (0, import_obsidian.normalizePath)(`${sourceDir}/${cleaned}`);
      const fromSourceDir = this.app.vault.getAbstractFileByPath(joined);
      if (fromSourceDir instanceof import_obsidian.TFile && this.isImageFile(fromSourceDir)) {
  ...
  buildImageNameIndex() {
    const now = Date.now();
    if (this.imageNameIndexCache && now - this.imageNameIndexCachedAt < 5e3) {
  ...
  async exportFileToCache(vaultRelativePath, request) {
  ...
  async exportFileToCacheImpl(vaultRelativePath, request) {
    ...
    const effectiveWidthPx = Number.isFinite(requestedWidth) && requestedWidth > 0 ? Math.max(400, Math.min(2600, Math.floor(requestedWidth))) : this.settings.widthPx;
    const requestedPageHeight = Number(request?.pageHeightPx);
    const effectivePageHeightPx = Number.isFinite(requestedPageHeight) && requestedPageHeight > 0 ? Math.max(300, Math.min(2600, Math.floor(requestedPageHeight))) : this.settings.pageHeightPx;
    const requestedTargetPage = Number(request?.targetPage);
    const effectiveTargetPage = Number.isFinite(requestedTargetPage) && requestedTargetPage >= 0 ? Math.floor(requestedTargetPage) : 0;
    const requestedRenderProfile = this.normalizeRenderProfile(request?.renderProfile, "auto");
    const effectiveRenderProfile = requestedRenderProfile === "auto" ? this.resolveAutoRenderProfile(request?.previewCols, request?.previewRows) : requestedRenderProfile;
    ...
    const effectiveReadabilityZoom = Number.isFinite(requestedReadabilityZoom) && requestedReadabilityZoom > 0 ? Math.max(0.7, Math.min(2.4, requestedReadabilityZoom)) : 1;
    const requestedPageTallness = Number(request?.pageTallness);
    const effectivePageTallness = Number.isFinite(requestedPageTallness) && requestedPageTallness > 0 ? Math.max(0.7, Math.min(2.4, requestedPageTallness)) : 1;
    const deviceDPR = typeof window !== "undefined" ? window.devicePixelRatio || 1 : 1;
    const basePixelRatio = quickMode ? Math.max(1, Math.min(1.2, this.settings.pixelRatio)) : effectiveRenderProfile === "quality" ? Math.max(Math.min(this.settings.pixelRatio, 2.5), 1.9) : Math.min(this.settings.pixelRatio, 1.45);
    const effectivePixelRatio = Math.min(Math.max(basePixelRatio, deviceDPR), 2.5);
    const effectiveRenderWaitMs = quickMode ? Math.min(this.settings.renderWaitMs, 80) : effectiveRenderProfile === "quality" ? Math.max(this.settings.renderWaitMs, 240) : Math.min(this.settings.renderWaitMs, 140);
    ...
    const ensureRenderedMarkdown = async () => {
      if (markdownPrepared) {
    ...
    const cacheRoot = this.resolveEffectiveCacheRoot();
    await this.ensureCacheDirs(cacheRoot);
    const computedDigest = (0, import_node_crypto.createHash)("md5").update(vaultRelativePath).digest("hex");
    ...
    const imgDir = import_node_path.default.join(cacheRoot, "img");
    const outputPath = import_node_path.default.join(cacheRoot, "img", `${digest}.png`);
    const debugPath = import_node_path.default.join(cacheRoot, "log", `${digest}.json`);
    const errorPath = import_node_path.default.join(cacheRoot, "log", `${digest}.error.json`);
    ...
    const reportStage = async (nextStage, extra = {}) => {
      stage = nextStage;
    ...
    try {
      ...
      if (!usedLiveView) {
        ...
        try {
          await this.withTimeout(
            import_obsidian.MarkdownRenderer.render(this.app, markdownForRender, host, vaultRelativePath, component),
            RENDER_TIMEOUT_MS,
            `Markdown render timed out at ${RENDER_TIMEOUT_MS}ms`
          );
        } catch (error) {
          const renderError = error instanceof Error ? { name: error.name, message: error.message, stack: error.stack ?? null } : { message: String(error) };
      ...
      if (this.settings.enableDebugLogs) {
        const firstParagraph = host.querySelector("p, li, h1, h2, h3, h4, h5, h6");
        const firstParagraphStyle = firstParagraph ? getComputedStyle(firstParagraph) : null;
        const firstBodyParagraph = host.querySelector("p, li");
        ...
        Object.assign(debugBase, {
          ...
          firstParagraphTextJustify: firstParagraphStyle ? firstParagraphStyle.textJustify ?? null : null,
          ...
          firstBodyParagraphTextJustify: firstBodyParagraphStyle ? firstBodyParagraphStyle.textJustify ?? null : null,
      ...
      if (this.settings.enableDebugLogs) {
        await import_node_fs.promises.writeFile(
          debugPath,
          JSON.stringify(
            { ...debugBase, at: (/* @__PURE__ */ new Date()).toISOString(), stage },
            null,
            2
          )
        );
      ...
      const sourceHeightLimit = configuredMaxHeight > 0 ? Math.min(sourceHeight, Math.max(1e3, configuredMaxHeight)) : sourceHeight;
      ...
      if (!quickMode && effectiveRenderProfile !== "quality") {
        if (sourcePixels > 18e6) {
          requestedScale = Math.min(requestedScale, 1.18);
        } else if (sourcePixels > 12e6) {
      ...
      if (appliedScale < MIN_CAPTURE_SCALE) {
        ...
        const safetyHeightLimit = Math.max(1e3, Math.min(maxHeightByBytes, maxHeightByEdge));
      ...
      try {
        canvas = await this.withTimeout(
          (0, import_html2canvas.default)(host, {
            backgroundColor: colors.backgroundColor,
            scale: appliedScale,
            useCORS: true,
      ...
      const keepPages = writeAllPages ? this.computeAllPages(pageCount) : this.computePagesToWrite(pageCount, targetPage, pageWriteRadius);
      await this.cleanupTempPageImages(imgDir, digest);
      const baseTempPath = `${outputPath}.tmp`;
      const pageTempPaths = /* @__PURE__ */ new Map();
      ...
      for (const i of pageIndexes) {
        ...
        const pageTempPath = import_node_path.default.join(imgDir, `${digest}--tmp-p${String(i).padStart(4, "0")}.png`);
        await import_node_fs.promises.writeFile(pageTempPath, pageBuffer);
        pageTempPaths.set(i, pageTempPath);
        if (i === 0) {
          await import_node_fs.promises.writeFile(baseTempPath, pageBuffer);
        }
      }
      ...
      for (const i of pageIndexes) {
        ...
        const finalPagePath = import_node_path.default.join(imgDir, `${digest}--p${String(i).padStart(4, "0")}.png`);
        await import_node_fs.promises.rename(tempPath, finalPagePath);
      }
      await import_node_fs.promises.rename(baseTempPath, outputPath);
      await this.cleanupObsoletePageImages(imgDir, digest, pageCount, keepPages);
      await reportStage("write-meta");
      const metaPath = import_node_path.default.join(imgDir, `${digest}.meta.json`);
      ...
      await import_node_fs.promises.rename(metaTempPath, metaPath);
      stage = "write-debug-final";
      if (this.settings.enableDebugLogs) {
        await import_node_fs.promises.writeFile(
          debugPath,
          JSON.stringify(
            { ...debugBase, at: (/* @__PURE__ */ new Date()).toISOString(), stage: "done", pageCount, pageHeight },
            null,
            2
          )
        );
      ...
      await import_node_fs.promises.unlink(errorPath).catch(() => void 0);
      return outputPath;
    } catch (error) {
  ...
  ensureSafeCacheRoot(cacheRoot) {
    ...
    if (!import_node_path.default.isAbsolute(raw)) {
      throw new Error(`Unsafe cache directory: must be absolute (${raw})`);
    }
    const resolved = import_node_path.default.resolve(raw);
    const normalized = import_node_path.default.normalize(resolved);
    const fsRoot = import_node_path.default.parse(normalized).root;
    const homeRoot = import_node_path.default.resolve((0, import_node_os.homedir)());
    if (normalized === fsRoot || normalized === homeRoot) {
      throw new Error(`Unsafe cache directory: ${normalized}`);
    }
    return normalized;
  }
  validateCacheRoot(value) {
    return this.ensureSafeCacheRoot(this.expandPath(value));
  }
  resolveEffectiveCacheRoot() {
  ...
  async ensureCacheDirs(cacheRoot) {
    const safeRoot = this.ensureSafeCacheRoot(cacheRoot);
    const dirs = [
      safeRoot,
      import_node_path.default.join(safeRoot, "img"),
      import_node_path.default.join(safeRoot, "mode"),
      import_node_path.default.join(safeRoot, "locks"),
      import_node_path.default.join(safeRoot, "log"),
      import_node_path.default.join(safeRoot, "requests"),
      import_node_path.default.join(safeRoot, "requests", "queue")
    ];
    for (const dir of dirs) {
      await import_node_fs.promises.mkdir(dir, { recursive: true, mode: 448 });
    }
    if (process.platform !== "win32") {
      for (const dir of dirs) {
        await import_node_fs.promises.chmod(dir, 448);
      }
    }
    const sentinelPath = import_node_path.default.join(safeRoot, ".obsidian-yazi-cache");
    await import_node_fs.promises.writeFile(sentinelPath, "obsidian-yazi-cache\n", { mode: 384 });
    if (process.platform !== "win32") {
      await import_node_fs.promises.chmod(sentinelPath, 384);
    }
  }
  computePagesToWrite(pageCount, targetPage, radius) {
  ...
  async cleanupObsoletePageImages(imgDir, digest, validPageCount, keepPages) {
    const files = await import_node_fs.promises.readdir(imgDir).catch(() => []);
    ...
    await Promise.all(
      files.map(async (name) => {
        if (name.startsWith(tempPrefix) && name.endsWith(".png")) {
          await import_node_fs.promises.unlink(import_node_path.default.join(imgDir, name)).catch(() => void 0);
          return;
        }
  ...
  async cleanupTempPageImages(imgDir, digest) {
    const files = await import_node_fs.promises.readdir(imgDir).catch(() => []);
    const tempPrefix = `${digest}--tmp-p`;
    await Promise.all(
      files.filter((name) => name.startsWith(tempPrefix) && name.endsWith(".png")).map((name) => import_node_fs.promises.unlink(import_node_path.default.join(imgDir, name)).catch(() => void 0))
    );
  }
  statusFilePath(cacheRoot, digest) {
    return import_node_path.default.join(cacheRoot, "log", `${digest}.status.json`);
  }
  async writeRenderStatus(cacheRoot, digest, payload) {
    ...
    await import_node_fs.promises.writeFile(tempPath, content).then(() => import_node_fs.promises.rename(tempPath, statusPath)).catch(async () => {
      await import_node_fs.promises.writeFile(statusPath, content).catch(() => void 0);
      await import_node_fs.promises.unlink(tempPath).catch(() => void 0);
    });
  }
  expandPath(value) {
    if (value.startsWith("~/")) {
      return import_node_path.default.join((0, import_node_os.homedir)(), value.slice(2));
    }
    return value;
  }
  resolveColors() {
...
.yazi-exporter-render-host .metadata-properties {
  ...
  shouldPreferForeignObjectRendering(host, quickMode, profile) {
    if (!quickMode) {
      return true;
    }
    if (profile === "quality") {
    ...
    const textSample = (host.textContent ?? "").slice(0, 12e3);
    if (textSample.length === 0) {
      return false;
    }
    return CJK_TEXT_RE.test(textSample);
  }
  applyReadabilityTuning(host, zoomRaw, baseWidthPx) {
    ...
    const compensationFactor = visualZoom < 1 ? Math.min(Math.pow(1 / visualZoom, 0.7), 1.35) : Math.max(0.85, 1 / Math.pow(visualZoom, 0.45));
  ...
  cloneLivePreview(host, vaultRelativePath) {
    ...
    const liveRoot = previewContainer.querySelector(".markdown-preview-sizer") ?? previewContainer.querySelector(".markdown-preview-view.markdown-rendered") ?? previewContainer;
  ...
  approximateDataUrlBytes(dataUrl) {
    return Math.max(0, Math.floor(dataUrl.length * 0.75));
  }
  evictInlineImageCacheToLimit(maxBytes = INLINE_IMAGE_CACHE_MAX_BYTES) {
  ...
  sweepInlineImageCache(now = Date.now()) {
  ...
  storeInlineImageCache(key, dataUrl) {
  ...
  async inlineLoadedImages(host, maxImages) {
    const images = Array.from(host.querySelectorAll("img"));
    const limit = Number.isFinite(Number(maxImages)) && Number(maxImages) > 0 ? Math.floor(Number(maxImages)) : Number.POSITIVE_INFINITY;
    let processed = 0;
  ...
  async waitForRenderStability(host, renderWaitMs = this.settings.renderWaitMs, frameWaitMs = 80) {
  ...
  async waitFrameOrTimeout(timeoutMs) {
    await Promise.race([
      new Promise((resolve) => requestAnimationFrame(() => resolve())),
      new Promise((resolve) => setTimeout(resolve, timeoutMs))
    ]);
  }
  async waitForFonts(maxWaitMs = 1200) {
    try {
  ...
  ensureRenderableFallback(host, markdown) {
    ...
    fallback.textContent = markdown.slice(0, 2e4);
...
var YaziExporterSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Cache directory").setDesc("PNG cache output directory (outside Vault recommended, absolute path required). If OBSIDIAN_YAZI_CACHE is set in Obsidian's environment, it overrides this value.").addText(
      (text) => text.setPlaceholder(DEFAULT_CACHE_DIR).setValue(this.plugin.settings.cacheDir).onChange(async (value) => {
        const next = value.trim() || DEFAULT_SETTINGS.cacheDir;
        try {
          this.plugin.validateCacheRoot(next);
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error);
          new import_obsidian.Notice(`Yazi Exporter: ${message}`);
          return;
        }
    ...
    new import_obsidian.Setting(containerEl).setName("Render width").setDesc("Rendered image width in pixels.").addText(
      (text) => text.setValue(String(this.plugin.settings.widthPx)).onChange(async (value) => {
        this.plugin.settings.widthPx = Math.max(500, Number(value) || DEFAULT_SETTINGS.widthPx);
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Max image height").setDesc("0 = unlimited. Set a cap only if very long notes should be clipped.").addText(
      (text) => text.setValue(String(this.plugin.settings.maxHeightPx)).onChange(async (value) => {
        ...
        if (!Number.isFinite(parsed)) {
          ...
          this.plugin.settings.maxHeightPx = Math.max(1e3, Math.floor(parsed));
        }
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Page height").setDesc("Height (px) of each split PNG page. Smaller values improve readability in yazi.").addText(
      (text) => text.setValue(String(this.plugin.settings.pageHeightPx)).onChange(async (value) => {
        this.plugin.settings.pageHeightPx = Math.max(300, Number(value) || DEFAULT_SETTINGS.pageHeightPx);
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Pixel ratio").setDesc("1.0-2.0 is recommended. Higher value makes sharper but larger PNG.").addText(
      (text) => text.setValue(String(this.plugin.settings.pixelRatio)).onChange(async (value) => {
        const parsed = Number(value);
        this.plugin.settings.pixelRatio = Math.max(1, Math.min(2.5, Number.isFinite(parsed) ? parsed : DEFAULT_SETTINGS.pixelRatio));
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Render wait ms").setDesc("Wait time after markdown render before PNG capture.").addText(
      (text) => text.setValue(String(this.plugin.settings.renderWaitMs)).onChange(async (value) => {
        this.plugin.settings.renderWaitMs = Math.max(0, Number(value) || DEFAULT_SETTINGS.renderWaitMs);
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Enable debug logs").setDesc("Recommended OFF. When ON, detailed debug logs are written under cache/log. Path fields are redacted by default; set OBSIDIAN_YAZI_DEBUG_INCLUDE_PATHS=1 only when explicitly needed.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.enableDebugLogs).onChange(async (value) => {
        this.plugin.settings.enableDebugLogs = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Use current theme colors").setDesc("ON: use current Obsidian theme colors in PNG output (recommended).").addToggle(
      (toggle) => toggle.setValue(!this.plugin.settings.forceCustomColors).onChange(async (value) => {
        this.plugin.settings.forceCustomColors = !value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Background color").setDesc("Background color used when 'Use current theme colors' is OFF.").addText(
      (text) => text.setPlaceholder("#ffffff").setValue(this.plugin.settings.backgroundColor).onChange(async (value) => {
        this.plugin.settings.backgroundColor = value.trim() || DEFAULT_SETTINGS.backgroundColor;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Text color").setDesc("Text color used when 'Use current theme colors' is OFF.").addText(
      (text) => text.setPlaceholder("#222222").setValue(this.plugin.settings.textColor).onChange(async (value) => {
